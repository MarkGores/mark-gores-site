...
- **Homepage market card** reads `src/data/market.json`, a snapshot of priorlake.realestate's public `/api/metrics` (same numbers that site shows), refreshed Mondays by `.github/workflows/refresh-market.yml` running `scripts/refresh_market.py` (commits only on change). Homes for sale / under contract / months of supply come from the API's `activeListings`, `pendingListings` and `absorptionRate`, safe since the Sept 25 2026 fix (before it they counted withdrawn `MlgCanView=0` records, about double the real inventory).
